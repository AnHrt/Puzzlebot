...
class GoToGoal():  
            ...
            if self.lidar_received: 
                ...
                if self.at_goal():  
                    ...
                    if ((self.closest_range <= fw_distance) and (abs(thetaFWC - thetaGTG)<=np.pi/2)): 
                        ...
                        d_H1 = np.sqrt((self.x_target-self.robot.x)**2+(self.y_target-self.robot.y)**2)
                    ...
                    if self.intersect_line(A,B,C, d_H1): 
    ...
    def intersect_line(self, A, B, C, d_H1):
        ...
        if((current_time - self.time_of_change) >= 3):
            x_actual = self.robot.x 
            y_actual = self.robot.y


            d = abs((A*x_actual)+(B*y_actual)+C)/np.sqrt((A**2)+(B**2))

            if(d < 0.1):
                d_L1 = np.sqrt((self.x_target-self.robot.x)**2+(self.y_target-self.robot.y)**2)
                if d_L1 < d_H1:
                    return True
                else :
                    return False
